...
function takeSnapShot() {
  ...
  //Criando o JPG
  var dataURI = canvas.toDataURL('image/png') //O resultado é um BASE64 de uma imagem.
...
function sendSnapShot(base64) {
  ...
  request.onload = function () {
    ...
    if (request.status >= 200 && request.status < 400) {
      ...
      document.querySelector('#caminhoImagem a').innerHTML = data.img.split(
        '/'
      )[1]
    } else {
      console.log('Erro ao salvar. Tipo:' + request.status)
    }
  }
